...
maths.int32mulinv = function (d) {          // d must be odd.
   var xn, t;
   if (d % 2 === 0) { throw "Error"; }
   xn = d;
   for(;;) {
	  t = maths.imul(d, xn);
	  if (t == 1) {return xn;}
	  xn = maths.imul(xn, 2 - t);
   }
}
...
maths.int64mulinv = function (d) {          // d must be odd.
   var xn, t;
   if (/[02468]($|\D\d*)/.test(""+d)) { throw "Error"; }
   xn = d;
   for(;;) {
	  t = maths.int64_mul(d, xn);
	  if (t == 1) {return xn;}
	  xn = maths.int64_mul(xn, maths.int64_sub(2, t));
   }
}
...
plugin.__MultiLine__ = function (str) {
	...
	if (remainLines === 0) {
		extendedFlag = false;
		maths_var_set(__VariableName, multiLineOutput);
		multiLineOutput = "";
		return "";
	}
}
...
var maths_var = function (str) {
	str = str.replace(/(^\s*)|(\s*$)/g, "");
	if (str.slice(0,6) === "#expr:") {
		return evalString(str.slice(6));
	} else {
		return maths_var_func(str);
	}
}
Object.defineProperty(maths, "!", {get: function(){return "\x7C";}});
var fromTemplate = function (str) {
	str = str.split(''); var newStrStack = [""], newStrPtr = 0, backupStr;
	for (var i = 0, j = str.length; i < j; ++i) {
		switch (str[i]) {
			case "{":
			if (str[i+1] === "{") {
				newStrPtr++;
				newStrStack[newStrPtr] = ""; ++i;
			} else {
				if (str[i+1] == null) {
					backupStr = "";
				} else {
			...
			if (str[i+1] === "}") {
				...
				newStrPtr--;
				newStrStack[newStrPtr] += maths_var(newStrStack[newStrPtr + 1]);
				newStrStack.length--; ++i;
			} else {
				if (str[i+1] == null) {
					backupStr = "";
				} else {
